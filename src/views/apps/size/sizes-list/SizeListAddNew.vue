<template>
  <b-sidebar
    id="add-new-size-sidebar"
    :visible="isAddNewSizeSidebarActive"
    bg-variant="white"
    ...
    right
    @hidden="resetForm"
    @change="(val) => $emit('update:is-add-new-size-sidebar-active', val)"
  >
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onSubmit = () => {
      store.dispatch("app-size/addSize", sizeData.value).then(() => {
        emit("refetch-data");
