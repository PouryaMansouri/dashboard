<template>
  <div>
    <stock-transfer-add-new
      :is-add-new-stock-transfer-sidebar-active.sync="
        ...
        <b-row>
          ...
          <b-col cols="12" md="6">
            <div class="d-flex align-items-center justify-content-end">
              ...
              <b-button
                variant="primary"
                @click="isAddNewStockTransferSidebarActive = true"
              >
                <span class="text-nowrap">Transfer Stock</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
...
import useStocksTransfer from "./useStocksTransfer";
import stockStoreModule from "../stockStoreModule";
import StockTransferAddNew from "./StockTransferAddNew.vue";
...
export default {
  ...
  setup() {
    ...
    if (!store.hasModule(Stock_APP_STORE_MODULE_NAME))
      store.transferModule(Stock_APP_STORE_MODULE_NAME, stockStoreModule);

    // UnTransfer on leave
    onUnmounted(() => {
      if (store.hasModule(Stock_APP_STORE_MODULE_NAME))
        store.untransferModule(Stock_APP_STORE_MODULE_NAME);
    });

    const isAddNewStockTransferSidebarActive = ref(false);
